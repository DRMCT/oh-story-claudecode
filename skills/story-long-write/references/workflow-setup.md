...
> **对标节奏回流（有主对标书时，卷纲定稿前做一次）**：读主对标 `对标/{书}/剧情/节奏.md`；缺失时设置 `missing_primary_contract: true` 并停止，提示重跑 `/story-long-analyze` Stage 3+ 或重新 `/story-import`，不得用章节摘要、故事线或拆文报告代替。文件存在时，按 references/outline-structure-theory.md「对标节奏迁移」把对标的一级结构关键点（1/4·中点·3/4）换素材排进本卷卷纲「对标结构坐标」——选段以主对标剧情单元为单位，按「类型/桥段标签」圈同类（详见该节步骤 1），选中的剧情单元写入剧情单元卡「对标剧情参照」。**「对标剧情参照」可填「自创：{理由}」**，内容形态不必照搬对标，仍过安全七检与批末结构复核。若尚未登记主对标书但 `拆文库/` 有同题材候选，先回 Phase 1「对标发现」登记再排节奏；确无对标书则按八节点占比自排。
...
3. **供给自查**：每批出细纲前检查六项：①对手戏由谁承接？具名对手间隔超过 2 章是复核提示；对手可为盟友或亲人，环境、关系章不为凑间隔新增反派。②未来 3 章出场角色的设定卡是否齐全？③能力增量与既有库存是否接得上？④本批引用的数字、制度、规矩、距离与脚程是否确实存在？引用前 grep 核对文件里的具体条目，不能只猜文件存在。⑤本批使用的地点空间属性先落档；仅提名不强补底图。是否与上一单元同场景、同对手、同胜负手连排，需要调整？⑥本批有哪些故事线推进，是否有可调存量？结果以「供给自查」小节落在本单元卡：B 级登记新增内容与设定路径，C 级列候选及影响面交作者裁定；无缺口写「无缺口」。并检查上一批「本章没写成的」与「建纲追加」里的未解决缺口。
...
|---|---|---|---|---|
| 1 | {主角在账单上发现4800元转出} | {信息揭示} | {密} | {禁：不揭露幕后者。放：他当场算出这笔钱等于几个月房租，允许写出来} |

  功能标签＝目的词：铺垫/高潮/爽点/打脸/人物塑造/设定，用于判断该点承担什么义务。点数由语义需要决定，不用 `目标字数 / beat 数`、固定字数档位或历史偏差系数反推。结构预检只判输入是否可安全执行，不宣称细纲一定能撑到目标字数。

  **分辨率表示展开程度，不是字数配额**：密（慢镜头逐拍展开）/ 中 / 疏（一句带过）。**全章不许全填「中」**；同类并列的点递减、不平均。留空按「中」处理。
...
- 复沓锚句：{只收必须一字不差重现的既有文本（律文、誓言、系统面板、旧案原话、前文已写出且要回声的句子），一行一条并注明落点，如"点3：立此为凭…"；人物的日常对白不进这里，也不在情节点里写成整句台词，只写这句话要做到什么，台词由写手按角色卡的腔调与"情绪过程"现写；没有写"无"}
...
2. **B 级·发明＋落档**：既有框架内的具名配角、复用地点的空间属性、能力形态、物价内插、规矩细则、刻度序列，可直接补充，不与既有裁定冲突。先增量落设定，再供细纲引用，并在《供给单》登记。项目引用用书目录相对路径（如 `设定/角色/张三.md`）；`./`、`../` 相对细纲目录，裸文件名不校验。地点只补本批用到的分区、进出口、相对位置或脚程。补纲授权下的新故事线先在卷纲故事线表分配 L 编号并写清主线对接口，再供细纲引用；日更不自动扩纲。
3. **C 级·提案裁定**：牵连全书的金手指规则、真相、伏笔、主线、力量档位、经济锚点、读者契约及马甲归因，不自行决定。《供给单》列具体缺口、1-2 个候选及关联设定影响，批末交作者；裁定先落设定再引用，不以占位或临时编数代替。推翻既有裁定、两个设定文件互相冲突也归 C 级，列出出处供作者判断。
...
- **追踪事务初始状态**：项目尚无正文且无 state 时，任何一批细纲落盘后都初始化；只讨论/只做大纲不执行，已有追踪不重建。先完整读取 [tracking-initialization.md](tracking-initialization.md)，构造第 0 章 JSON，执行 `scripts/tracking_commit.py init` 后 `check`，通过后删除事务 JSON；核心角色动态快照在首次正文事务中创建。
